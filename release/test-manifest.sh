...
expect_fail "$BUILD" --version 1.2.3 --channel staging --endpoint-alias deepwind-staging \
  --endpoint-url https://dev.deepwind.ai/mcp --key-id test-key \
  --not-before 2026-01-01T00:00:00Z --not-after 2027-01-01T00:00:00Z \
  --source-revision deadbeef --archive "claude=$TMP/traversal.tar.gz" --output "$TMP/bad.json"
expect_fail "$BUILD" --version 1.2.3 --channel staging --endpoint-alias deepwind-staging \
  --endpoint-url https://dev.deepwind.ai/mcp --key-id test-key \
  --not-before 2026-01-01T00:00:00Z --not-after 2027-01-01T00:00:00Z \
  --source-revision deadbeef --archive "claude=$TMP/duplicate/deepwind-harness-claude-v1.2.3.tar.gz" --output "$TMP/bad.json"
expect_fail "$BUILD" --version not-semver --channel staging --endpoint-alias deepwind-staging \
  --endpoint-url https://dev.deepwind.ai/mcp --key-id test-key \
  --not-before 2026-01-01T00:00:00Z --not-after 2027-01-01T00:00:00Z \
  --source-revision deadbeef --archive "claude=$TMP/deepwind-harness-claude-v1.2.3.tar.gz" --output "$TMP/bad.json"
expect_fail "$BUILD" --version 1.2.3 --channel staging --endpoint-alias deepwind-staging \

...
usage: build-manifest.sh --version SEMVER --channel staging|production --endpoint-alias NAME \
  ...
  --source-revision HEX --archive target=/absolute/archive.tar.gz [--archive ...] --output FILE
EOF
exit 2; }

VERSION= CHANNEL= ENDPOINT_ALIAS= ENDPOINT_URL= KEY_ID= NOT_BEFORE= NOT_AFTER= REVISION= OUTPUT=
ARCHIVES=()
while [ "$#" -gt 0 ]; do
  case "$1" in
    --version|--channel|--endpoint-alias|--endpoint-url|--key-id|--not-before|--not-after|--source-revision|--output)
      [ "$#" -ge 2 ] || usage
      case "$1" in
        ...
        --not-after) NOT_AFTER=$2;; --source-revision) REVISION=$2;; --output) OUTPUT=$2;;
      esac
      shift 2;;
...
[ -n "$KEY_ID" ] && [ -n "$NOT_BEFORE" ] && [ -n "$NOT_AFTER" ] && [ -n "$REVISION" ] && [ -n "$OUTPUT" ] || usage
...
jq -nS \
  ...
  '{formatVersion:1,version:$version,tag:$tag,channel:$channel,endpoint:{alias:$alias,url:$url},signing:{keyId:$key,notBefore:$notBefore,notAfter:$notAfter,signatureFile:"deepwind-release-manifest.json.asc"},provenance:{repository:"DeepWindAI/harness",revision:$revision},archives:($archives|sort_by(.target))}' > "$tmp"
